...
static int
target_value_upward_or_downward_elev (double floor, int value) {
    if(floor > value) {
        return DOWNWARD;
    } else if (floor < value){
        return UPWARD;
	...
	switch(priority) {
		case UPWARD:
          for (int i = 0; i < MAX_N_ELEVATORS; i++) {
              if (((unsigned long)elevators[i].addr.s_addr != 0) && (floor < elevators[i].floor) && (elevators[i].priority == UPWARD)) {
