...
static int
orderlist_update_remote_elev (struct in_addr addr, elevstatus_t* s) {
...
orderlist_sync_external_orders (message_t *m) {
    ...
    printf("syncing external orders\n");

	// som sync funksjonen funker nå fungere ikke sletting av targets bra, menn ikke så bra like vell.
    for (int i = 0; i < N_FLOORS-1 ; i++) {
    	...
    	/* Har ikke testet din kode med for løkka
        // UP 
        ...
        if (!m->callUp[i].updated && !callUp[i].updated) {
          callUp[i].registered = m->callUp[i].registered || callUp[i].registered;
        }
        ...
        if (!m->callDown[i].updated && !callDown[i].updated) {
          callDown[i].registered = m->callDown[i].registered || callDown[i].registered;
        }
		*/
}
...
orderlist_is_local_elev_synced (void) {
	for (int i = 0; i < MAX_N_ELEVATORS; i++) {
		if ((unsigned long)elevators[i].addr.s_addr != 0 && elevators[i].synced == false) {
...
orderlist_add_elev_ready_to_sync(struct in_addr ext_elev_ready_for_sync){
	...
	for(int i = 0; i < MAX_N_ELEVATORS; i++){
		if ((unsigned long)elevators[i].elev_ready_for_sync.s_addr == 0){
...
elevstatus_t*
orderlist_get_local_status (void) {
...
orderlist_sync (int fd) {
    ...
    while (!synced && !orderlist_global_sync()) {
        //usleep(1000);
        for (int i = 0; i < 2;i++) {
            ...
            */
			/*
            printf("\033[2J\033[1;1H"); orderlist_print_lists();
            printf("1\n"); printf("synced %d\n",synced);
			*/
            if (numbytes > 0) { printf("numbytes: %d\n", numbytes);
		...
		synced = orderlist_is_local_elev_synced();
